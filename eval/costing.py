...
def render(summary: dict[str, Any], *, cap_usd: float) -> str:
    ...
    lines += [
        ...
        "## Reading it",
        "",
        "The completion side dominates nothing here — this system is **prompt-heavy**: every",
        "proposal carries the state slice, and a lock exports its facts into every subsequent",
        "prompt. That is the cost consequence of the design being about state rather than",
        "about generation, and it is why a cheaper prompt rate matters more than a cheaper",
        "completion rate when picking the chunk-7 model.",
        "",
